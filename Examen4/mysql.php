...
function conexion() {
    ...
    if (!$errno == 0) {
        $mensaje.= "<h2>No se ha podido establecer la conexión con el servidor. Se ha producido un error nº $errno que corresponde a: $error</h2>";
    }
    return $conexion;
}

function cerrarConexion() {
    global $conexion, $mensaje;
    $operacion = true;
    if (!@mysqli_close($conexion)) {
        $mensaje.= "<h2> No se ha podido cerrar la conexión</h2>";
    }
}
