...
function displayFormLogin($camposErroneos = array(), $camposPendientes = array()) {
    ...
    $datos = array(
        "error" => $error,
        "enlace" => "<a href='index.php?registrarse'>Registrarse</a>",
...
function displayFormRegistro($camposErroneos = array(), $camposPendientes = array(), $duplicado = false) {
    ...
    if ($camposPendientes || $camposErroneos || $duplicado) {
        ...
        if ($camposErroneos) {
            $error.= '<p class="error">Hubo algunos problemas con el formulario que usted presentó. Por favor, introduzca valores adecuados (usuario y password mínimo 5 caracteres y máximo 15) en los campos y pulse Registrar.</p>';
        }
        if ($duplicado) {
