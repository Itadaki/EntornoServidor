...
function procesForm() {
    ...
    $query = "INSERT INTO " . TABLA .
            ...
            " VALUES(?, ?, ?, ?, ?, ?)";
    if ($conexion = conexion() && $consulta = mysqli_stmt_init($conexion)) {
        $consulta = mysqli_stmt_init($conexion);
        mysqli_stmt_prepare($consulta, $query);
        mysqli_stmt_bind_param($consulta, 'ssisss', $nombre, $sexo, $edad, $sistema, $aficiones, $futbol);
        mysqli_execute($consulta);
        $errorNo = mysqli_stmt_errno($consulta);
        $errorMsg = mysqli_stmt_error($consulta);
        if (!$errorNo) {
